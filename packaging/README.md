...
|----------|----------------|---------------------------------------------|
| AUR      | Arch Linux     | [`aur/getyaak/PKGBUILD`](aur/getyaak/PKGBUILD), [`aur/getyaak-bin/PKGBUILD`](aur/getyaak-bin/PKGBUILD) |
...
- **`getyaak`** — builds from source using `cargo`. Install build-time
  dependencies and run `makepkg -si` inside `packaging/aur/getyaak`.
- **`getyaak-bin`** — downloads the upstream prebuilt Linux `x86_64` tarball
...
(`aur.archlinux.org/getyaak.git` and `aur.archlinux.org/getyaak-bin.git`).

```sh
cd packaging/aur/getyaak
